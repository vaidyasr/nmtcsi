...
APPINIT_VERSION_URL="https://nmt.myhass.us/csi/csi-downloads/appinit_version"
APPINIT_UPGRADE_URL="https://nmt.myhass.us/csi/csi-downloads/appinit.cgi"
...
{
    file="$1"
    url=`echo "$1"|cut -d':' -f1|grep -c 'http'`
    if [ "$url" -ne 0 ];then
...
application_info()
{
    ...
    TEMP="`echo "$TEMP"| sed '/^{/d' | sed 's| =|=|g' |sed '/^}/d'|sed 's/,*$//g'| sed 's/$/,/g' |sed '$s/,$//'|sed -e 's/^[ \t]*//'|sed 's|^|"|g'|sed 's|=|"=|g'|sed 's|=|:|g'`"
    echo "$TEMP"
}
...
process()
{
    ...
    APP_COUNT=0
    IFS=$(echo -en "\n\b")
    DATA=`ls -1d $APPS_FOLDER/*/appinfo.json 2>/dev/null`
    NO_OF_APPS=`echo "$DATA"|grep -c "appinfo.json"`
    for appinfo in `echo "$DATA"`
    do
            ...
            if [ -z "$parameter" ] || [ "$parameter" == "$namelower" ]; then
                ...
                if [ "$enabled" == "1" ] || [ -n "$2" ] || [ "$1" == "info" ]; then
                    ...
                    case "$1" in
                        ...
                        info)
                        if [ $APP_COUNT -eq 0  -a "$parameter" = "" ];then
                          echo "["
                        fi
                        echo "{"
                        application_info
                        APP_COUNT=$(($APP_COUNT + 1))
                        if [ $APP_COUNT -eq $NO_OF_APPS ];then
                          echo "}"
                          echo "]"
                        elif [ "$parameter" != "" ];then
                          echo "}"
...
#Check command
case "$command" in
    install|uninstall|start|stop|restart|info|setuplog|daemonlog|rescan|enable|disable|webserver_enable|webserver_disable)
    ;;

    *)
    echo
    echo "Usage: "
    echo "# $APPINIT_FILENAME {start|stop|restart|setuplog|daemonlog|uninstall|rescan|enable|disable} [application name]"
...
if [ -z "$parameter" ]; then
    case "$command" in
        uninstall|enable|disable|install|setuplog|daemonlog)
...
if [ "$process" == "1" ]; then
    ...
    case "$command" in
         ...
         daemonlog)
         process "daemon" "$parameter"
         ;;

         *)
         process "$command" "$parameter"
         if [ "$command" != "info"  -a "$command" != "uninstall" ];then
